...
export default function SetupNotificationsPage() {
  ...
  const handleContinue = async () => {
    setSaving(true);
    try {
      await onboardingApi.setStep('integrations');
  ...
  return (
    <div>
      <div style={{ marginBottom: 8, fontSize: 13, color: accentBrand, fontWeight: 600 }}>Step 10</div>
      <h1 style={{ fontSize: 24, fontWeight: 800, marginBottom: 8 }}>Notification Settings</h1>
      <p style={{ fontSize: 15, color: textSecondary, marginBottom: 28 }}>
        Agent assignment, user updates, SLA breach alerts. Configure in Admin after setup.
      </p>
